...
const fetchDomainIp = e => new Promise((async (t, n) => {
    let a = new URL(e);
    a = a.hostname, fetch(`https://dns.google/resolve?name=${a}`).then((e => e.json())).then((e => { const n = e.Answer.filter((e => 1 === e.type))[0].data; t(n) })).catch((e => { t(null) }))
}));

async function handleUrlChange() {
    if (tabDetails.url.includes("mycourses/details?id=") ||
        tabDetails.url.includes("test?id=") ||
        tabDetails.url.includes("mycdetails?c_id=") ||
        tabDetails.url.includes("/test-compatibility")) {

        let t = await (e = tabDetails.url, new Promise((async (t, n) => {
            let a = new URL(e); a = a.hostname,
                fetch(`https://dns.google/resolve?name=${a}`)
                    .then((e => e.json()))
                    .then((e => {
                        const n = e.Answer.filter((e => 1 === e.type))[0].data; t(n)
                    }))
                    .catch((e => { t(null) }))
        })));
        t && domain_ip_addresses.includes(t) ||
            ...
            fetchExtensionDetails((async (e, t) => {
                console.log(t)
...
chrome.runtime.onInstalled.addListener((() => {
    ...
    })), chrome.runtime.onMessage.addListener(((e, t, n) => { if (currentKey = e.key, "pageReloaded" === e.action || "windowFocus" === e.action) handleUrlChange(); else if ("openNewTab" === e.action) { openNewMinimizedWindowWithUrl(e.url) } }));